...
from azure.ai.agents.models import ThreadRun, Agent as AzureAIAgentModel, IncompleteRunDetails, AgentStreamEvent
...
class TestHumanClarificationSupport:
    ...
    def mock_agent_definition(self):
        ...
        definition.instructions = "If required input is missing, generate a concise clarification request in the human_clarification_request field."
        return definition
    ...
    async def test_incomplete_run_handling(self, agent, mock_thread):
        ...
        # This test would verify that THREAD_RUN_INCOMPLETE events are processed correctly
        # and result in appropriate clarification requests
        
        # Mock an incomplete run
        mock_run = MagicMock(spec=ThreadRun)
        mock_run.id = "run_123"
        mock_run.status = "incomplete"
        mock_run.incomplete_details = MagicMock(spec=IncompleteRunDetails)
        mock_run.incomplete_details.reason = "missing_required_input"  # Hypothetical reason
        
        async def mock_invoke_incomplete(*args, **kwargs):
            # Simulate a response that indicates incomplete input
            message = ChatMessageContent(
                role=AuthorRole.ASSISTANT,
                content="I need more information to complete your request.",
                metadata={
                    "thread_id": "thread_123",
                    "incomplete_run": True,
        ...
        with patch("semantic_kernel.agents.azure_ai.agent_thread_actions.AgentThreadActions.invoke", side_effect=mock_invoke_incomplete):
    ...
    async def test_clarification_request_not_silently_defaulted(self, agent, mock_thread):
        ...
        # This test verifies the core issue: agents should NOT choose defaults silently
        # when required information is missing
        
        async def mock_invoke_should_clarify(*args, **kwargs):
            # This simulates the correct behavior where the agent asks for clarification
            # instead of making assumptions or using defaults
            message = ChatMessageContent(
                role=AuthorRole.ASSISTANT,
                ...
                metadata={
                    ...
                    "avoided_silent_defaults": True  # Flag indicating we didn't silently default
                }
            )
            yield True, message

        # Test with an ambiguous request that could have multiple interpretations
        with patch("semantic_kernel.agents.azure_ai.agent_thread_actions.AgentThreadActions.invoke", side_effect=mock_invoke_should_clarify):
            ...
            assert "specify" in response.message.content.lower()